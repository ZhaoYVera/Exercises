...
test_graph = CompleteGraph.build_random_complete_graph(5)

pi0 = [0 for _ in range(test_graph.n)]
...
def update_graph_weight(graph, pi):
    ...
    for i in range(graph.n):
        for j in range(i, graph.n):
            d[i][j] = d[j][i] = graph.adj_mat[i][j] + pi[i] + pi[j]
...
def build_m1t(graph):
    ...
    return [length, degree, last_edge[1], tree, vertices]
...
# print(weighted_total_weight(test_graph, pi0))
# print(alpha_nearness(test_graph))


def best_pi(graph):
    pi = [ 0 for _ in range(graph.n)]
    w_0 = weighted_total_weight(graph, pi)[0]
    degree_0 = weighted_total_weight(graph, pi)[1]
    v_0 = [d - 2 for d in degree_0]
    v_1 = v_0[:]
    t = 1
    v_optimal = [0 for _ in range(graph.n)]
    while t > 0.0001 and v_1 != v_optimal:

        pi_t = [pi[i] + t*(0.7*v_0[i]+0.3*v_1[i]) for i in range(graph.n)]
        w_1 = weighted_total_weight(graph, pi_t)[0]

        degree_1 = weighted_total_weight(graph, pi)[1]

        while v_1 != v_optimal and w_1 > w_0:
            v_1 = v_0[:]
...
# print(best_pi(test_graph))
# print(weighted_total_weight(test_graph, best_pi(test_graph)))
# print(alpha_nearness(update_graph_weight(test_graph, best_pi(test_graph))))
